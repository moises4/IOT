...
len0 = length(allPixelData);
normalizedData = [];

for i = 1:len0
    colData = ( allPixelData(:,i) - mean(allPixelData(:,i)) ) / std(allPixelData(:,i));
    normalizedData = [normalizedData, colData];
end
...
for i = 1:totalDataSize 
    ...
    reshapedImage = reshape(normalizedData(i, 1:end), [48,48])'; % Reshape image into 48x48 matrix
